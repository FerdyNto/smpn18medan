...
@section('content')
    <div class="d-flex justify-content-between my-3">
        <h2 class="fs-2 m-0">Berita Sekolah</h2>
        <a href="{{ route('berita.create') }}" class="btn btn-primary fs-5"><span><i
                    class="fa-regular fa-square-plus"></i></span> Tambah Berita</a>
    </div>

    <form action="{{ route('berita.index') }}" method="GET" class="col-lg-4">
                ...
                <tbody>
                    ...
                    @foreach ($berita as $b)
                        <tr>
                            ...
                            <td class="p-2">
                                <a href="{{ route('berita.edit', ['slug' => $b->slug]) }}"
                                    class="btn btn-warning text-white"><i class="fa-solid fa-pen-to-square"></i></a>
                            </td>
                            <td class="p-2">
                                {{-- <form action="{{ route('berita.delete', ['slug', $b->slug]) }}" method="POST"
                                    onsubmit="return confirm('Yakin ingin hapus berita dengan judul {{ $b->judul }} ???')"
                                    class="d-inline">
                                    @csrf
                                    @method('delete')
                                    <button type="submit" class="btn btn-danger" data-confirm-delete="true"><i
