...
@section('content')
    <div class="d-flex justify-content-between my-3">
        <a class="btn btn-secondary" href="{{ route('info.index') }}" role="button">Kembali</a>
    </div>
    <h2 class="my-5">Form Info Baru</h2>

    <form action="{{ route('info.store') }}" method="POST" enctype="multipart/form-data" class="col-11">
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            <label for="isi_berita" class="form-label">Isi Info</label>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" aria-label="Kategori" name="id_kategori">
                ...
                @foreach ($kategori as $item)
                    <option value="{{ $item->id }}" @if ($item->id === 2) selected @endif>
    ...
    <script>
        CKEDITOR.replace('isi_berita', {
            height: 300,
            filebrowserUploadUrl: "{{ route('upload.gambar.info', ['_token' => csrf_token()]) }}",
