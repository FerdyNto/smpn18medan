...
@section('content')
    <div class="d-flex justify-content-between my-3">
        <h2 class="fs-2 m-0">Data Kewirausahaan (Jasa)</h2>
        <a href="{{ route('jasa.create') }}" class="btn btn-primary fs-5"><span><i
                    class="fa-regular fa-square-plus"></i></span> Tambah Produk Baru</a>
    </div>

    <form action="" method="GET" class="col-lg-4">
        <div class="input-group mb-3">
            <input type="text" class="form-control" placeholder="Cari Jasa" name="jasa" value="{{ old('jasa') }}">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-sm">
            <thead>
                <tr>
                    <th scope="col">#</th>
                    <th scope="col" class="col-2">Foto Jasa</th>
            ...
            <tbody>
                ...
                @foreach ($usaha as $u)
                    <tr>
                        <td>{{ $no++ }}</td>
                        <td><img src="{{ asset('img/jasa') . '/' . $u->gambar }}" alt="{{ $u->nama }}" class="img-ds">
                        </td>
                        <td>{{ $u->nama }}</td>
                        <td>{!! $u->deskripsi !!}</td>
                        <td>Rp. {{ $u->harga }}</td>
                        ...
                        <td>{{ $u->kategori }}</td>
                        <td>
                            <a href="{{ route('jasa.edit', ['slug' => $u->slug]) }}" class="btn btn-warning text-white"><i
                                    class="fa-solid fa-pen-to-square"></i></a>
                        </td>
                        <td>
                            {{-- <form action="{{ route('jasa.delete', ['slug' => $u->slug]) }}" method="POST"
